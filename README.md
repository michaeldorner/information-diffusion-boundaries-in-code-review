...
The communication networks are in the subfolder `data/networks`, the simulation results are stored in `data/minimal_paths`
...
Because of the large runtime of the simulation, we provide precomputed results of the simulation via [Zenodo](). You can download the results and place in the subfolder `data/minimal_paths`. Consider verify the `.pickle` and `.csv` files (see previous section).
